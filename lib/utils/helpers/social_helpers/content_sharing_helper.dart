...
class ContentSharingHelper {
  ...
  static Future<bool> _shareWithImage({
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse(imageUrl),
        headers: {'User-Agent': 'Dabbler-App'},
  ...
  static String _formatForTwitter(String text, String? link) {
    ...
    if (tweetText.length > maxLength) {
      final availableLength = maxLength - (link?.length ?? 0) - 5; // 5 for newline + ellipsis
  ...
  static String _formatForEmail(String text, String authorName, String? link) {
    var emailBody = 'Hi!\\n\\n$text\\n\\n';
    emailBody += 'I thought you might be interested in connecting with $authorName ';
    emailBody += 'on Dabbler, the sports social app where athletes and enthusiasts ';
