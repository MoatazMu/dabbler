  ...
  Failure? _validateSearchParameters(FindGamesParams params) {
    ...
    if (params.startDate != null && params.endDate != null) {
      if (params.startDate!.isAfter(params.endDate!)) {
        return const GameFailure(message: 'Start date cannot be after end date');
      }
    }

    // Validate location coordinates
    if ((params.userLatitude != null) != (params.userLongitude != null)) {
      return const GameFailure(message: 'Both latitude and longitude must be provided for location-based search');
    }

    if (params.userLatitude != null) {
      if (params.userLatitude! < -90 || params.userLatitude! > 90) {
        return const GameFailure(message: 'Invalid latitude. Must be between -90 and 90');
      }
    }

    if (params.userLongitude != null) {
      if (params.userLongitude! < -180 || params.userLongitude! > 180) {
        return const GameFailure(message: 'Invalid longitude. Must be between -180 and 180');
      }
    }

    // Validate radius
    if (params.radiusKm != null && params.radiusKm! <= 0) {
      return const GameFailure(message: 'Radius must be greater than 0');
    }
    ...
    // Validate pagination
    if (params.page < 1) {
      return const GameFailure(message: 'Page must be greater than 0');
    }

    if (params.limit < 1 || params.limit > 100) {
      return const GameFailure(message: 'Limit must be between 1 and 100');
    }
